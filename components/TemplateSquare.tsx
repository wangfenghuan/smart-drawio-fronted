...
interface TemplateSquareProps {
    initialMaterials?: API.MaterialVO[]
    initialPagination?: {
        current: number
        pageSize: number
        total: number
...
export default function TemplateSquare({ initialMaterials = [], initialPagination = { current: 1, pageSize: 12, total: 0 } }: TemplateSquareProps) {
    ...
    // Only load initial data if NOT provided by SSR
    useEffect(() => {
        if (initialMaterials.length === 0 && initialPagination.total === 0) {
            loadMaterials()
        }
    }, [])
    ...
    // Handle pagination change without full page reload for smoother UX
    // Ideally we could push router state here too
